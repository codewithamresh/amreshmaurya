    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        <div className="relative">
          ...
          <h2 className={`text-5xl md:text-6xl font-black tracking-tight ${textClass}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            >
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div className="space-y-4">
                  <div className={`flex items-center gap-2 text-xs font-bold tracking-widest uppercase ${isDark ? "text-emerald-400" : "text-emerald-600"}`}>
                  ...
                  {post.tags && (
                    <div className="flex flex-wrap gap-2">
                      {post.tags.slice(0, 3).map(tag => (
                        <span key={tag} className={`text-[10px] px-2.5 py-1 rounded-md border ${isDark ? "border-slate-700 bg-slate-800 text-slate-400" : "border-slate-200 bg-slate-50 text-slate-500"}`}>
                          #{tag}
                        </span>
                      ))}
                    </div>
                ...
                <div className={`p-3 rounded-2xl transition-all duration-500 ${isDark ? "bg-slate-800 group-hover:bg-emerald-500" : "bg-slate-100 group-hover:bg-emerald-500"}`}>
                   <ArrowUpRight className={`w-5 h-5 transition-colors duration-500 ${isDark ? "text-emerald-500 group-hover:text-white" : "text-slate-600 group-hover:text-white"}`} />
                </div>
              </div>

              {/* Bottom Row: Content */}
              <div className="relative z-10 mt-12">
                <h3 className={`${isMain ? "text-3xl md:text-5xl" : "text-2xl"} font-bold mb-4 tracking-tight leading-[1.1] ${textClass}`}>
                  {post.title}
                </h3>
                <p className={`${subTextClass} ${isMain ? "text-xl" : "text-base"} line-clamp-3 mb-8 max-w-2xl`}>
                  {post.description}
                </p>
                
                <div className="flex items-center gap-2 font-bold text-sm tracking-wide group-hover:gap-4 transition-all duration-300">
                  <span className={isDark ? "text-white" : "text-slate-900"}>READ ARTICLE</span>
        ...
        >
          ...
          <h4 className={`text-xl font-bold mb-2 ${textClass}`}>Curious for more?</h4>
          <p className={`${subTextClass} mb-6 text-sm`}>Explore my full collection of technical guides and stories.</p>
